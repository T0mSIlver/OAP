 ...
 *********************************************/
 ...
 subject to {
   forall (j in dist)
     	demandCl : sum (i in brew) x[i][j] >= demand[j]; //pour chaque DC, on lui ship plus de biere que sa demande
   forall (i in brew) 
   		capacityBrew : sum (j in dist) x[i][j] <= capacityB[i]; //pour chaque brasserie, on ship moins de biere que la capacité à produire de la biere de la brasserie
   forall (i in brew)
     	beerYield: sum (j in dist) x[i][j] <= sum (p in plant) y[p][i]* maltY[p];
   	 	//beerYield: sum (j in dist) x[i][j] <= (y[1][i]+y[2][i])*8.333 + y[3][i]*9.091;  ---old version à la main
   	    //pour chaque brasserie, on ship moins de bière que le malt recu (avec conversion MT malt -> ML bière)
   	    //remarque : on pourrait faire des boucles au lieu d'iterer a travers les plants à la main pour convertir les MT malt en ML beer --- done
   forall (p in plant)
     	capacityMalt : sum (i in brew) y[p][i] <= capacityM[p];  //pour chaque plant, on ship moins de malt que la capacité à produire
 }
 ...
 execute{
  ...
  /* writeln("Reduced cost of shipping from Istanbul to Antalya: ", x[1][3].reducedCost)
   ...
   writeln("-----------------------------------------------------------------------------------")
   for(var j in dist)
   		writeln("Dual var: Demand at DC ",j,": ", demandCl[j].dual)
